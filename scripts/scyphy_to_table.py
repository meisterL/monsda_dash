...
def parse_args():
    ...
    parser.add_argument(
        "-d", "--fls_dir", action="store", type=argparse.FileType("r"), nargs="+"
    )
...
def print_pkl_file(df, out_dir):
    # os.mkdir(out_dir)
    print("print pkl")
    file = open(os.path.join(out_dir, "peakTable_DF.pkl"), "wb")
    pickle.dump(df, file)
    file.close()
...
def print_tsv_file(df, out_dir):
    # os.mkdir(out_dir)
    print("print csv")
    df.to_csv(os.path.join(out_dir, "peakTable.csv"), sep=",")
